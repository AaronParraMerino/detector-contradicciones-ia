...
export default function App() {
  ...
  const detect = useCallback(async () => {
    ...
    try {
      ...
      setResult(data);
      setHistory((h) => [
        { premise: premise.trim(), hypothesis: hypothesis.trim(), ...data, ts: Date.now() },
        ...h.slice(0, 9),
      ]);
    ...
    async (accepted) => {
      ...
      try {
        ...
        if (data.ready_to_retrain)
          alert(`🎉 ¡${data.total_corrections} correcciones! El modelo está listo para reentrenarse.`);
      } catch (e) {
        setError(e.message);
      } finally {
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-12 space-y-10">
        ...
        <header className="text-center space-y-3">
          ...
          <p className="text-white/50 text-base max-w-md mx-auto leading-relaxed">
            Detecta contradicciones entre enunciados con aprendizaje por refuerzo.
          </p>
        </header>
            ...
            {[
              {
                ...
                value: premise,
                set: setPremise,
                placeholder: "El cielo es azul durante el día.",
              },
              {
                id: "hypothesis",
                label: "Hipótesis",
                value: hypothesis,
                set: setHypothesis,
                placeholder: "El cielo es completamente negro a mediodía.",
              },
          ...
          <div ref={resultsRef} className="animate-fade-in-up glass rounded-2xl p-6 space-y-6">
            ...
            {!feedbackSent ? (
              ...
              >
                ...
                {totalCorrections !== null && (
                  <span className="text-white/40 text-xs ml-auto">{totalCorrections} correcciones totales</span>
                )}
              </div>
            )}
          </div>
